...
const OPENSSLDIR_PATTERN = /OPENSSLDIR\s*\:\s*\"([^\"]*)\".*/;
const CA_CERTIFICATES_PATTERN = /.+.(pem|crt|cer)$/i;
...
function addCertificates(cb){
  return function(err, certificates) {
    ...
    if (!cas.__added){
      ...
      console.log('Total number of CAs', cas.length);
      cas.__added = true;
    }
    cb();
  }
...
function injectSystemCA(cb) {
  if (process.platform === 'win32') {
    ...
    exec('openssl version -d', function(err, stdout, stderr){
      ...
      var match = OPENSSLDIR_PATTERN.exec(stdout);
      if (match && match.length > 1) {
        ...
        fs.readdir(certDir, function(err, files) {          
            ...
            files = files
              .filter(function (file){ return CA_CERTIFICATES_PATTERN.test(file);})
              .map(function(file) {
